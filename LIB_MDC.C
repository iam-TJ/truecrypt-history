...
int mdcshsInitEx( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	...
	cryptInfo->privateDataLength = sizeof( CRYPT_INFO_MDCSHS );
...
#else
  #define shsTransform(cI)	bigToLittleLong( ( LONG * ) cI->currentIV, SHS_DIGESTSIZE ); \
							SHSTransform( ( LONG * ) cI->currentIV, ( LONG * ) cI->key ); \
							bigToLittleLong( ( LONG * ) cI->currentIV, SHS_DIGESTSIZE );
#endif /* _BIG_WORDS */
...
int mdcshsInitKey( CRYPT_INFO *cryptInfo )
	...
	bigToLittleLong( ( LONG * ) cryptInfo->transUserKey, SHS_DATASIZE );
...
int mdcshsInitIV( CRYPT_INFO *cryptInfo )
	...
	bigToLittleLong( ( LONG * ) cryptInfo->currentIV, CRYPT_MAX_IVSIZE );
