...
int rc5InitEx( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	{
	...
	/* Allocate memory for the key and the algorithm-specific data within
	   ...
	   cryptInfoExPtr->noRounds at this point since we set things up when we
	   perform the rc5InitKey() function, as the number of rounds is key-
	   dependant */
	if( cryptInfo->key != NULL || cryptInfo->privateData != NULL )
		return( CRYPT_INITED );
	if( ( status = secureMalloc( &cryptInfo->key, sizeof( RC5_KEY ) ) ) != CRYPT_OK )
		return( status );
	if( cryptInfoExPtr->rounds == CRYPT_USE_DEFAULT )
		cryptInfo->privateUseDefaults = TRUE;
	if( ( status = secureMalloc( &cryptInfo->privateData, sizeof( CRYPT_INFO_RC5 ) ) ) != CRYPT_OK )
		{
		secureFree( cryptInfo->key );
		return( status );
		}
	setRC5info( cryptInfo, cryptInfoExPtr->rounds );
	cryptInfo->keyLength = sizeof( RC5_KEY );
...
int rc5Init( CRYPT_INFO *cryptInfo )
	{
	CRYPT_INFO_RC5 cryptInfoEx;

	/* Use the default number of rounds */
	memset( &cryptInfoEx, 0, sizeof( CRYPT_INFO_RC5 ) );
	cryptInfoEx.rounds = CRYPT_USE_DEFAULT;

	/* Pass through to the extended setup routine */
	return( rc5InitEx( cryptInfo, &cryptInfoEx ) );
	}
...
int rc5EncryptCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	...
	while( blockCount-- )
		...
		memcpy( cryptInfo->currentIV, buffer, RC5_BLOCKSIZE );
...
int rc5DecryptCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	...
	while( blockCount-- )
		...
		memcpy( cryptInfo->currentIV, temp, RC5_BLOCKSIZE );
...
int rc5EncryptCFB( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	...
	while( noBytes )
		...
		/* Encrypt the IV */
		rc5encrypt( rc5Key, cryptInfo->currentIV );
	...
	cryptInfo->ivCount = ( ivCount % RC5_BLOCKSIZE );
...
int rc5EncryptPCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	RC5_KEY *rc5Key = ( RC5_KEY * ) cryptInfo->key;
	BYTE *plaintextChain = cryptInfo->currentIV + RC5_BLOCKSIZE;
	BYTE temp[ RC5_BLOCKSIZE ];
	int blockCount = noBytes / RC5_BLOCKSIZE;
	...
	while( blockCount-- )
		...
		memcpy( temp, plaintextChain, RC5_BLOCKSIZE );
		memcpy( plaintextChain, buffer, RC5_BLOCKSIZE );
...
int rc5DecryptPCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	RC5_KEY *rc5Key = ( RC5_KEY * ) cryptInfo->key;
	BYTE *plaintextChain = cryptInfo->currentIV + RC5_BLOCKSIZE;
	BYTE temp[ RC5_BLOCKSIZE ];
...
int getRC5info( const CRYPT_INFO *cryptInfo )
	{
	return( ( ( CRYPT_INFO_RC5 * ) cryptInfo->privateData )->rounds );
	}

void setRC5info( CRYPT_INFO *cryptInfo, const int rounds )
	{
	( ( CRYPT_INFO_RC5 * ) cryptInfo->privateData )->rounds = rounds;
	}
...
int rc5InitKey( CRYPT_INFO *cryptInfo )
	{
	CRYPT_INFO_RC5 *cryptInfoEx = ( CRYPT_INFO_RC5 * ) cryptInfo->privateData;
	RC5_KEY *rc5Key = ( RC5_KEY * ) cryptInfo->key;

	/* If the number of rounds has been preset, use this value */
	rc5Key->noRounds = ( cryptInfoEx->rounds != CRYPT_USE_DEFAULT ) ? \
					   cryptInfoEx->rounds : RC5_DEFAULT_ROUNDS;

	rc5keyInit( rc5Key, cryptInfo->userKey, cryptInfo->userKeyLength );
	return( CRYPT_OK );
	}
