...
#include <memory.h>
  ...
  #include "bnlib/legal.h"

  #include "bnlib/kludge.h"
#endif /* Compiler-specific includes */
...
volatile const char * volatile const lbnCopyright_64 = bnCopyright;
...
{
	lbnZero_64(out, len);
	BIGLITTLE(*(out-len),*(out+len)) = lbnMulAdd1_64(out, in, len, k);
}
...
lbnFromMont_64(BNWORD64 *n, BNWORD64 *mod, unsigned len)
{
	...
	lbnMontReduce_64(n, mod, len, lbnMontInv1_64(BIGLITTLE(mod[-1],mod[0])));
