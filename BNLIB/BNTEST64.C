...
#define F BIGLITTLE((f+SIZE),f)
	...
	if (modbits) {
		...
		for (j = 0; j < 10; j++) {
			...
			bnput64("a = ", A, i);
			lbnExpMod_64(A, A, i, C, i, B, i);
			bnput64("a^(p-1) mod p = ", A, i);
		}
		...
		for (j = 0; j < 25; j++) {
			...
			*(BIGLITTLE(E-l,E+l-1)) |= (t >> 1) + 1;
			...
			gettime(&start);
#endif
			lbnDoubleExpMod_64(D, A, i, B, k, D, i, E, l, C, i);
#if CLOCK_AVAIL
			gettime(&stop);
	...
	for (j = 1; j <= 1000; j++) {
		...
		for (i = 1; i <= SIZE/2; i++) {
			...
			/* D = A + B * t */
			carry = lbnAddN_64(D,A,i);

			/* For comparison with the result of lbnMulAdd1_64 */
			...
			*(BIGLITTLE(A-i-1,A+i)) = 0;
			...
			for (l = 0;
			     ...
			     l++)
			{
				t = lbnModQ_64(z > 0 ? C : D,
				               (unsigned)(z > 0 ? z : -z),
				               smallprimes[l]);
				carry = lbnModQ_64(A, i, smallprimes[l]);
				borrow = lbnModQ_64(B, i, smallprimes[l]);
				if (!t != (!carry && !borrow)) {
					bnput64("a = ", A, i);
					printf("a mod %u = %lu\n",
					       smallprimes[l],
