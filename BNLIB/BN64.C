...
#else
  #include "bnlib/lbn.h"
  #include "bnlib/lbn64.h"
  #include "bnlib/lbnmem.h"
  #include "bnlib/bn64.h"
  #include "bnlib/bn.h"

  /* Work-arounds for some particularly broken systems */
  #include "bnlib/kludge.h"	/* For memmove() */
#endif /* Compiler-specific includes */
...
unsigned
bnLSWord_64(struct BigNum const *src)
{
	return src->size ? (unsigned)((BNWORD64 *)src->ptr)[BIGLITTLE(-1,0)]: 0;
}

unsigned
bnBits_64(struct BigNum const *src)
{
	return lbnBits_64((BNWORD64 *)src->ptr, src->size);
}
...
bnModQ_64(struct BigNum const *src, unsigned d)
{
	...
	return lbnModQ_64((BNWORD64 *)src->ptr, s, d);
}
...
bnGcd_64(struct BigNum *dest, struct BigNum const *a, struct BigNum const *b)
{
	        ...
	        lbnCmp_64((BNWORD64 *)b->ptr, (BNWORD64 *)a->ptr, asize) > 0))
	{
		...
		if (i >= 0) {
			...
			lbnCopy_64((BNWORD64 *)dest->ptr, tmp,
				   (unsigned)-i);
			dest->size = (unsigned)-i;
		}
	} else {
...
bnLShift_64(struct BigNum *dest, unsigned amt)
{
	...
	if (amt % 64) {
		carry = lbnLshift_64(dest->ptr, s, amt % 64);
		if (carry) {
			s++;
			bnSizeCheck(dest, s);
...
void bnRShift_64(struct BigNum *dest, unsigned amt)
{
	...
	if (amt)
		(void)lbnRshift_64(dest->ptr, s, amt);

	dest->size = lbnNorm_64(dest->ptr, s);
	MALLOCDB;
}
