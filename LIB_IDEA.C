...
int ideaInitEx( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	...
	if( cryptInfo->key != NULL || cryptInfo->privateData != NULL )
		return( CRYPT_INITED );
	if( ( status = secureMalloc( &cryptInfo->key, IDEA_EXPANDED_KEYSIZE ) ) != CRYPT_OK )
		return( status );
...
int ideaEncryptPCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	IDEA_KEY *ideaKey = ( IDEA_KEY * ) cryptInfo->key;
	BYTE *plaintextChain = cryptInfo->currentIV + IDEA_BLOCKSIZE;
	BYTE temp[ IDEA_BLOCKSIZE ];
	int blockCount = noBytes / IDEA_BLOCKSIZE;
	...
	while( blockCount-- )
		...
		memcpy( temp, plaintextChain, IDEA_BLOCKSIZE );
		memcpy( plaintextChain, buffer, IDEA_BLOCKSIZE );
...
int ideaDecryptPCBC( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	IDEA_KEY *ideaKey = ( IDEA_KEY * ) cryptInfo->key;
	BYTE *plaintextChain = cryptInfo->currentIV + IDEA_BLOCKSIZE;
	BYTE temp[ IDEA_BLOCKSIZE ];
